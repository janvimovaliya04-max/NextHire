...
export default function BrowseJobs() {
  ...
  const filteredByCategory = useMemo(() =>
    browseJobs.filter((job) => activeFilter === "All" || job.category === activeFilter),
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, columnId, filterValue) => {
      ...
      return (
        ...
        job.company.toLowerCase().includes(v) ||
        job.location.toLowerCase().includes(v) ||
        job.tags.some((tag) => tag.toLowerCase().includes(v))
      );
    },
  });
          ...
          {["All", "Engineering", "UI/UX"].map((filter) => (
            <Button
              ...
              sx={{
                ...
                "&:hover": {
                  borderColor: primary,
                  bgcolor: activeFilter === filter ? (secondary || primary) : `${primary}14`,
                }
              }}
            >
              {filter}
      ...
      >
        <InfiniteScroll
          ...
          endMessage={
            <Typography
              align="center"
              sx={{ py: { xs: 2, sm: 3 }, color: subText, fontWeight: 600 }}
            >
